...
function VirtualClassRoom(props) {
	...
	function init_video(role) {
		...
		connection = new signalR.HubConnectionBuilder()
			.withUrl(`${host}/p/Courses/3fa85f64-5717-4562-b3fc-2c963f66afa6/Classrooms/${classID}/join`)
			.build();
		setConnection(connection);
		navigator.mediaDevices
			.getUserMedia(role === 'Instructor' ? { video: true, audio: true } : { audio: true })
			.then((stream) => {
				myStream = stream;
	...
	function addNewUser([ id, info ]) {
		...
		const user = {
			[id]: { call_id: id, connected: false, audio: info.HasAudio, video: info.HasVideo, username: info.UserName }
		};
	...
	function createScreen() {
		navigator.mediaDevices.getDisplayMedia().then((screenStream) => {
			let screenConnection = new signalR.HubConnectionBuilder()
				.withUrl(
					`http://127.0.0.1:51044/p/Courses/3fa85f64-5717-4562-b3fc-2c963f66afa6/Classrooms/${classID}/join`
				)
				.build();
			let stillAlive = true;
			...
			screenConnection.start().then(() => {
				...
				screenPeer.on('open', (id) => {
					...
					addNewUser([
						id,
						{ Role: 'Instructor', HasAudio: false, HasVideo: false, UserName: 'My Screen Share' }
					]);
	...
	useEffect(
		() => {
			...
			setTeacherIds(
				Object.entries(teacherDict).map(([ id, obj ]) => ({
					id,
									...
									{Object.entries(studentDict).map(([ id, user ]) => (
										<ListGroupItem key={id} style={{ backgroundColor: '#110033' }}>
											<ListGroupItemText>
												{user.username}
												{role === 'Instructor' &&
												mutelist[id] === undefined && (
													<FaMicrophoneAlt
														onClick={() => sendMute(id)}
														style={{ float: 'right', size: '3em' }}
													/>
												)}
												{role === 'Instructor' &&
												mutelist[id] !== undefined && (
													<FaMicrophoneAltSlash
														onClick={() => sendUnmute(id)}
														style={{ float: 'right', size: '3em' }}
													/>
												)}
